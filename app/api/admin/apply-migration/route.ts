...
export async function POST() {
  try {
    ...
    // The Supabase JS client doesn't support raw DDL execution
    // This will likely fail, but let's try via RPC if available
    ...
    if (error) {
      return NextResponse.json(
        {
          error: 'Cannot execute DDL via Supabase REST API',
          message: error.message,
          instructions: 'Please apply migration via Supabase Dashboard SQL Editor'
        },
        { status: 400 }
      );
    }
...
export async function GET() {
  return NextResponse.json({
    message: 'Use POST to apply migration',
    instructions: 'Or apply manually via Supabase Dashboard SQL Editor',
